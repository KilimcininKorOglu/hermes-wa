# HERMESWA - WhatsApp Multi-Instance API (Go/Golang)
...
- **Instance availability control** — `used` flag for external app integration, `description` for notes/tracking
...
| `APP_LOGIN_USERNAME` | Username for dashboard/API login | - | `hermeswa` |
...
For webhook security, HERMESWA signs every outgoing webhook (when a secret is configured for the instance) using an HMAC:
...
```powershell
# Build API Server
go build -o hermeswa.exe main.go
...
```powershell
# Build for Linux
$env:GOOS="linux"; $env:GOARCH="amd64"; go build -o hermeswa main.go
...
## 📸 Screenshots / Gallery
Here are some previews of the HERMESWA interface.
